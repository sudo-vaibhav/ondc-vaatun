...
export const gatewayRouter = router({
  ...
  onSearch: publicProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (transactionId) {
        await addSearchResponse(kv, transactionId, input as Parameters<typeof addSearchResponse>[2]);
      } else {
  ...
  onSelect: publicProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (transactionId && messageId) {
        await addSelectResponse(kv, transactionId, messageId, input as Parameters<typeof addSelectResponse>[3]);
      } else {
