...
## Context
SigNoz provides the trace backend for viewing distributed traces. It uses ClickHouse for storage and exposes an OTLP collector on port 4831 (HTTP). We're creating a minimal docker-compose setup with 4 services: ClickHouse (database), OTel Collector (trace ingestion), Query Service (API), and Frontend (UI). All ports use the 4830-4836 range to avoid clashes with other projects. This runs independently from the BAP server and can be started/stopped as needed for development. Prerequisites: Docker Desktop must be running.
...
**Action**: create
**Why**: Configure the OpenTelemetry Collector to receive OTLP traces and export to ClickHouse. Internal container ports are standard (4317/4318); host-mapped ports are 4831/4832.
...
services:
  ...
  otel-collector:
    ...
    ports:
      - "4832:4317"   # OTLP gRPC
      - "4831:4318"   # OTLP HTTP (used by our app)
    depends_on:
      - clickhouse
...
### Task 3: Add SigNoz pnpm scripts to root package.json
**File**: `package.json`
**Action**: modify
**Why**: Provide convenient scripts to manage SigNoz lifecycle.

Add these scripts to the root `package.json` scripts section:

```json
"signoz:up": "docker compose -f docker-compose.signoz.yml up -d",
"signoz:down": "docker compose -f docker-compose.signoz.yml down",
"signoz:clean": "docker compose -f docker-compose.signoz.yml down -v",
"signoz:logs": "docker compose -f docker-compose.signoz.yml logs -f"
```

### Task 4: Add SigNoz instructions to README
**File**: `README.md`
**Action**: modify
...
|---------|-------------|
| `pnpm signoz:up` | Start SigNoz (4 containers) |
| `pnpm signoz:down` | Stop SigNoz |
| `pnpm signoz:clean` | Stop + delete all trace data |
| `pnpm signoz:logs` | Tail container logs |

**SigNoz UI:** http://localhost:4830
...
```bash
curl -I http://localhost:4831/v1/traces
```
