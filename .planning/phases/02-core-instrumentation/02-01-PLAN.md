...
must_haves:
  truths:
    ...
    - "All 18 procedures (health, registry, gateway, results) are instrumented without per-router changes"
  artifacts:
...
Purpose: This is the foundation for the span hierarchy (HTTP request -> tRPC procedure -> ONDC action -> HTTP client). All 18 tRPC procedures will automatically get spans without per-router changes.
...
- All existing routers work without modification (they import publicProcedure which now includes middleware)
