        ...
        <Card className="border-2 border-foreground shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
            <CardHeader className="pb-2">
                <div className="flex items-center justify-between">
                    ...
                    {ttl && (
                        <Badge variant="outline" className="border-2 border-foreground flex items-center gap-1">
                    ...
                    {addOnItems.length > 0 && (
                        <>
                            ...
                            {addOnItems.map((item, index) => (
                                <div key={`addon-${index}`} className="flex justify-between items-center py-1 pl-4">
                                    <span className="text-sm text-muted-foreground">
                                        {item.item?.add_ons?.map(a => a.id).join(', ') || 'Add-on'}
                                    </span>
                                    <span className="text-sm font-medium">
                                        {formatCurrency(item.price.value, item.price.currency)}
                                    </span>
