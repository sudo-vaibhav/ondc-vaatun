    ...
    const handleTrigger = async () => {
        ...
        try {
            ...
            if (response.ok && data.transactionId) {
                ...
                throw new Error(data.message || "Failed to trigger search - no transaction ID returned");
            }
        } catch (error) {
            console.error("[SearchTrigger] Error:", error);
            setError(error instanceof Error ? error.message : "Failed to connect to the search endpoint");
        } finally {
            setIsLoading(false);
        }
    };
