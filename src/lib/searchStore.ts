...
export function addSearchResponse(
    ...
    if (!entry) {
        console.warn(`[SearchStore] No entry found for transaction: ${transactionId}`);
    ...
    console.log(`[SearchStore] Added response for transaction: ${transactionId} (total: ${targetEntry.responses.length})`);
...
export function getSearchResults(transactionId: string): {
    ...
    for (const response of entry.responses) {
        ...
        if (!providerMap.has(bppId)) {
            ...
            const providerName = response.message?.catalog?.providers?.[0]?.descriptor?.name;
            const itemCount = response.message?.catalog?.providers?.reduce(
